use crate::{AuthSession, AuthStore};
    ...
    ResBody::Error: Into<BoxError>,
{
    ...
    fn call(&mut self, mut req: Request<ReqBody>) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            let session = match req.extensions().get::<AxumSession>().cloned() {
            ...
            let current_id = if let Some(id) = session.get::<i64>("user_auth_session_id").await {
                ...
                self.store.anonymous_user_id
            };

            let user = {
                let users = self.store.inner.read().await;
